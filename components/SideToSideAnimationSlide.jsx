...
function SideToSideAnimationSlide() {
    ...
    return (
        <div className="w-full max-w-4xl h-128 bg-white rounded-lg shadow-xl flex">
            ...
            <div className={"w-1/2 h-full rounded-lg flex justify-center items-center gap-2 text-white transition-transform duration-500 overflow-hidden " 
            + "bg-gradient-to-br from-green-400 to-blue-400 " + (formState == 'Register' ? "-translate-x-full" : "")}>
                <div className={"w-full flex items-center flex-col -translate-x-full transition-transform duration-500 " 
                + (formState == 'Register' ? "translate-x-0" : "hidden")}>
                ...
                <div className={"w-full flex items-center flex-col transition-transform duration-500 " 
                + (formState == 'Register' ? "hidden translate-x-full" : "translate-x-0")}>
                    <h1 className="text-3xl font-bold mb-3">{formState === 'Register' ? registerSideTitle : loginSideTitle}</h1>
                    <p className={'max-w-xs text-center text-gray-100 transition-transform duration-500 '}>
                        {formState === 'Register' ? registerSideText : loginSideText}
                    </p>
